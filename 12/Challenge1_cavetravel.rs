...
fn main() {
	...
	let mut open : VecDeque<(String, &str)> = VecDeque::new();
	open.push_back((String::from("start"), "start"));
	loop {
		if open.len() == 0 {break}
		let current : (String, &str) = open.pop_front().unwrap();
		...
		for d in destinations {
			...
			if d.chars().any(|x| x.is_lowercase()){
				if !current.0.contains(d) {open.push_back((new_path, d))}
			}
			else {
				open.push_back((new_path, d))
			}
		}
	}
	for route in &routes {
